...
typedef enum { DP_STRAIGHT, DP_LEFT, DP_RIGHT } dirproj_e;
...
boolean P_CheckMissileRange(mobj_t *actor) // 80010C10
{
	...
	if (actor->flags &
	    MF_JUSTHIT) { /* the target just hit the enemy, so fight back! */
		actor->flags &= ~MF_JUSTHIT;
		return true;
	}

	if (gameskill >= sk_nightmare &&
	    I_Random() < 16) // [Immorpher] Early attack on nightmare
	{
...
dirtype_t diags[] = { DI_NORTHWEST, DI_NORTHEAST, DI_SOUTHWEST,
		      DI_SOUTHEAST }; // 8005AD24
...
boolean P_LookForPlayers(mobj_t *actor, boolean allaround) // 8001115C
{
	...
	if (!mo || mo->health <= 0 || !(actor->flags & MF_SEETARGET)) {
		if (actor->type >
		    MT_PLAYERBOT3) /* for monsters with immorpher fun for intro at skill 6 */
		{
		...
		} else /* special case for player bots */
		{
			...
			for (mobj = mobjhead.next; mobj != &mobjhead;
			     mobj = mobj->next) {
				...
				dist = P_AproxDistance(mobj->x - actor->x,
						       mobj->y - actor->y);
				if (!(dist < dist2))
					continue;
...
void A_Look(mobj_t *actor) // 80011340
{
	...
	if (actor->info->seesound) {
		...
		case sfx_possit3: //sfx_posit3:
			sound = sfx_possit1 +
				P_Random() %
					3; // [Immorpher] integer rounding to have 0,1,2 and restore missing sounds
			break;
...
void A_Chase(mobj_t *actor) // 8001146C
{
	...
	if (!actor->target || !(actor->target->flags &
				MF_SHOOTABLE)) { /* look for a new target */
	...
	if ((gameskill >= sk_nightmare || !actor->movecount) &&
	    actor->info->missilestate && P_CheckMissileRange(actor)) {
		P_SetMobjState(actor, actor->info->missilestate);
		if (gameskill < sk_nightmare)
			actor->flags |= MF_JUSTATTACKED;
		else if (I_Random() <
			 16) // [Immorpher] slim chance in nightmare mode to break attack for unpredictability
			actor->flags |= MF_JUSTATTACKED;
		return;
	}
...
void A_Scream(mobj_t *actor) // 80011740
{
	...
	case sfx_posdie3: //sfx_podth3:
		sound = sfx_posdie1 +
			P_Random() %
				3; // [Immorpher] now the third death sound happens given integer rounding
		break;
...
void A_PainShootSkull(mobj_t *actor, angle_t angle) // 8001267C
{
	...
	prestep = (mobjinfo[MT_SKULL].radius + (4 * FRACUNIT) +
		   actor->info->radius) >>
		  FRACBITS;
	...
	// Check for movements.
	if (!P_PathTraverse(actor->x, actor->y, newmobj->x, newmobj->y,
			    PT_ADDLINES, PIT_PainCheckLine) ||
...
{
	if (!(actor->target) || (actor->target->health <= 0) ||
	    !(P_AproxDistance(actor->target->x - actor->x,
...
void A_RectGroundFire(mobj_t *actor) // 8001296C
{
	...
	mo->target = actor;
	an = R_PointToAngle2(actor->x, actor->y, actor->target->x,
			     actor->target->y);
...
void A_RectMissile(mobj_t *actor) // 80012B1C
{
	...
	x = (finesine[an] * 68);
	y = (finecosine[an] * 68);
	mo = P_SpawnMobj(actor->x + x, actor->y - y, actor->z + (68 * FRACUNIT),
			 MT_PROJ_RECT);
	mo->target = actor;
	...
	x = (finesine[an] * 50);
	y = (finecosine[an] * 50);
	mo = P_SpawnMobj(actor->x + x, actor->y - y,
			 actor->z + (139 * FRACUNIT), MT_PROJ_RECT);
	mo->target = actor;
	mo->tracer = actor->target;
	mo->threshold = 1;
	an = (actor->angle + ANG270);
...
void A_BarrelExplode(mobj_t *actor) // 80013070
{
	...
	P_SpawnMobj(actor->x, actor->y, actor->z + (actor->info->height >> 1),
		    MT_EXPLOSION1);
