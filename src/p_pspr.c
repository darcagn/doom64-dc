...
void P_MovePsprites(player_t *player) // 8001B14C
{
	...
	player->psprites[ps_flash].sx = player->psprites[ps_flashalpha].sx =
		player->psprites[ps_weapon].sx;
	player->psprites[ps_flash].sy = player->psprites[ps_flashalpha].sy =
...
void P_RecursiveSound(sector_t *sec, int soundblocks) // 8001B254
{
	...
	for (i = 0; i < sec->linecount; i++) {
		...
		if (front->floorheight >= back->ceilingheight ||
		    ...
		    back->floorheight ==
			    back->ceilingheight) // [d64] added this check
...
void A_WeaponReady(player_t *player, pspdef_t *psp) // 8001B83C
{
	...
	angle = (64 * ticon) &
		(FINEANGLES - 1); // PsxDoom/D64 use ticon no gamevbls
...
{
	if (gameskill ==
	    sk_nightmare) // [Immorpher] double weapon lower speed on nightmare
	{
		psp->sy += 2 * LOWERSPEED;
	} else {
		psp->sy += LOWERSPEED;
...
void A_Raise(player_t *player, pspdef_t *psp) // 8001BA84
{
	...
	if (gameskill ==
	    sk_nightmare) // [Immorpher] double weapon raise speed on nightmare
	{
		psp->sy -= 2 * RAISESPEED;
	} else {
		psp->sy -= RAISESPEED;
...
void P_BulletSlope(mobj_t *mo) // 8001BF88
{
	...
	if (!linetarget) {
		...
		if (!linetarget) {
			an -= 2 << 26;
			bulletslope =
				P_AimLineAttack(mo, an, 0, 16 * 64 * FRACUNIT);
		}
	}
}
...
void A_BFGSpray(mobj_t *mo) // 8001C560
{
	...
	for (i = 0; i < 40; i++) {
		...
		P_SpawnMobj(linetarget->x, linetarget->y,
			    linetarget->z + (linetarget->height >> 1),
			    MT_BFGSPREAD);
		damage = 0;
		for (j = 0; j < 15; j++) {
			damage += (P_Random() & 7) + 1;
...
void P_LaserCrossBSP(int bspnum, laserdata_t *laser) // 8001C710
{
	...
	while (!(bspnum & NF_SUBSECTOR)) {
		...
		/* traverse nodes */
		ds1 = (((x1 - nx) >> FRACBITS) * ndy) -
		      (((y1 - ny) >> FRACBITS) * ndx);
		ds2 = (((x2 - nx) >> FRACBITS) * ndy) -
		      (((y2 - ny) >> FRACBITS) * ndx);
		...
		/* new child laser */
		childlaser = (laserdata_t *)Z_Malloc(sizeof(*childlaser),
						     PU_LEVSPEC, 0);
...
void A_FireLaser(player_t *player, pspdef_t *psp) // 8001CAC0
{
	...
	for (i = 0; i < lasercount; i++) {
		slopez = P_AimLineAttack(mobj, angleoffs, LASERAIMHEIGHT,
					 LASERRANGE);

		if (aimfrac)
			laserfrac =
				(aimfrac << (FRACBITS - 4)) - (4 << FRACBITS);
		else
		...
		/* setup laser */
		laser_data = (laserdata_t *)Z_Malloc(sizeof(*laser_data),
						     PU_LEVSPEC, 0);
		...
		laser_data->dist = 0;
		laser_data->distmax =
			(fixed_t)sqrtf((float)((x * x) + (y * y) + (z * z)));
